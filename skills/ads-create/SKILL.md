...
Hard rules that override default concept generation:
- Pick from the 7 disguise archetypes (tweet, native-OS, news chyron, plain-text billboard, apology, handwritten, branded testimonial) before designing
- Brand is demoted or absent (default = no logo on the static)
- Max 3 colors, extreme contrast (black/white default)
- Hook = specific number + timeframe and/or identity callout, in caps
- Value prop stack = outcome + timeframe + risk reversal + negatives
- Copy length is either 25–50 words (billboard) or 120–250 (story). Never mid-length
- CTA is demoted — never the loudest element
- Run the 11-item QA checklist (in the memory file) before any concept goes into `campaign-brief.md`. If 2+ items fail, regenerate that concept.
