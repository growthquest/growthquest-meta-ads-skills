...
- Default = no brand logo in the image. Only render the logo if the brief explicitly calls for it (and the archetype is testimonial or news).
- Max 3 colors. Pure black bg + white text is the default. Extreme contrast always.
- Hook in ALL CAPS, heaviest weight available, top 25% of canvas.
- Tweet mockups MUST render the real X (Twitter) Chirp typeface, verified checkmark, and avatar styling — pixel realism is the trick.
- Native-OS mockups (iOS Notes, Calendly, iMessage) must be pixel-accurate to the real UI. If banana can't render the UI faithfully, regenerate with a tighter prompt before delivering.
- CTA is demoted — never the largest element. If the generated image has a giant button competing with the hook, regenerate.
- If the rendered image could be recognized as a Meta ad in the first 0.5 seconds, the disguise failed and the image must be regenerated.
